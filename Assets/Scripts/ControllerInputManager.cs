...
public class ControllerInputManager: MonoBehaviour {
    ...
    private void OnTriggerStay(Collider other) {
        if (other.gameObject.CompareTag("Throwable")  || 
            other.gameObject.CompareTag("Structure") ||
              other.gameObject.CompareTag("Illegal"))  {
    ...
    private void ThowObject(Collider other) {
        RaycastHit platformRay;
        //Debug.Log("yes");
        if (Physics.Raycast(transform.position, -Vector3.up, out platformRay, 5, laserMask)) {
            if (platformRay.transform.gameObject.name == "Platform") {
